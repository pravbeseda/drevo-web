...
describe('HighlightPipe', () => {
    ...
    it('should highlight matching text', () => {
        spectator = createPipe(
            `<span [innerHTML]="'This is a test string' | highlight:'test'"></span>`
        );

        expect(spectator.element.innerHTML).toContain(
            '<mark class="highlighted-text">test</mark>'
        );
    });

    it('should highlight multiple occurrences', () => {
        spectator = createPipe(
            `<span [innerHTML]="'test one test two test' | highlight:'test'"></span>`
        );
    ...
    it('should preserve original case in highlighted text', () => {
        spectator = createPipe(
            `<span [innerHTML]="'TEST Test test' | highlight:'test'"></span>`
        );
    ...
    it('should not modify text when search is empty', () => {
        spectator = createPipe(
            `<span [innerHTML]="'Original text' | highlight:''"></span>`
        );
    ...
    it('should escape regex special characters in search term', () => {
        spectator = createPipe(
            `<span [innerHTML]="'This has (test) in it' | highlight:'(test)'"></span>`
        );
    ...
    it('should handle text with no matches', () => {
        spectator = createPipe(
            `<span [innerHTML]="'No match here' | highlight:'xyz'"></span>`
        );
    ...
    it('should return empty string for null input', () => {
        spectator = createPipe(
            `<span [innerHTML]="null | highlight:'test'"></span>`
        );
    ...
    it('should return empty string for undefined input', () => {
        spectator = createPipe(
            `<span [innerHTML]="undefined | highlight:'test'"></span>`
        );
    ...
    it('should handle whitespace in search term', () => {
        spectator = createPipe(
            `<span [innerHTML]="'test content' | highlight:'   '"></span>`
        );
    ...
    it('should escape HTML in text to prevent XSS', () => {
        spectator = createPipe(
            `<span [innerHTML]="'<script>alert(1)</script>' | highlight:'script'"></span>`
        );
    ...
    it('should handle HTML special characters in search term', () => {
        spectator = createPipe(
            `<span [innerHTML]="'a < b & c > d' | highlight:'<'"></span>`
        );
