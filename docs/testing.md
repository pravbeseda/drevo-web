...
2. **No selector ever appears in a spec.** `locator()`, `getByTestId()`, `getByRole()`, `getByText()`, `keyboard.*` and `mouse.*` all belong in a Page Object (`pages/`) or a helper (`helpers/`) — on `page` and on a locator a PO returned alike: narrowing `editPage.editorContainer.locator('.cm-panel-lint')` in a spec is the same violation, so the PO exposes `lintPanel` instead. A spec calls PO members and nothing else. `no-restricted-properties` in `testing/playwright/eslint.config.mjs` enforces it for `tests/**/*.spec.ts`, so `yarn lint:playwright` fails on the first one
...
- `nx test` is transpile-only (ts-jest): specs and source with real type errors pass it green, and `yarn lint` catches neither. Two commands do the type check the test run does not — `yarn nx build client --configuration=development` for the shipped code, and `yarn lint:typecheck` for the specs and helpers the build never compiles ([quality-gates.md](quality-gates.md)). Run both after touching a shared type or signature: the errors that slip past Jest are a newly required interface field missing from a hand-built mock, and a wrong function signature for a framework type (an Angular validator must be `(control: AbstractControl) => ValidationErrors | null`)
