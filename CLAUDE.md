...
11. **Every file carries its own floor** — beside `global`, each project's `coverageThreshold` holds glob keys, and a glob threshold applies **per file**: a new component or service with no spec fails on its own instead of being absorbed by an aggregate over hundreds of files. Three things about those keys are easy to get wrong:
    - **They resolve against `process.cwd()`**, which Nx sets to the workspace root — so a key reads `./libs/ui/src/…`, not `./src/…`. A key that matches nothing is reported as `Coverage data for … was not found` and gates nothing
    - **A glob group is subtracted from `global`.** If some files match no glob, `global` silently narrows to just those. Every project's globs are therefore written to partition all of its files, which is what keeps `global` an aggregate over the whole project
    - **A glob beats an exact-path key**, so a per-file exception cannot be written as its own entry — it has to fall outside the pattern, which is why the exclusions live inside the glob as `!(a|b)`
    Two carve-outs exist, both at a floor of 0: declaration-only modules (route tables, DI config, model and constant declarations — nothing to exercise) and named gaps that are genuinely untested (tracked in #267). A new file belongs to neither; give it a spec
12. **No `TODO`/`FIXME` comments** — `sonarjs/todo-tag` and `sonarjs/fixme-tag` are errors, so such a comment fails lint and blocks the commit. Either finish the work now or open a GitHub issue for it; when the code needs the context, write a plain comment stating the constraint and referencing the issue number
13. **Delete the code a change orphans** — when a refactor or a deletion leaves an export, file or dependency with no consumer, remove it in the same change. `yarn knip` finds them; it runs blocking in CI, so leaving them behind fails the PR anyway
14. **Type-coverage thresholds only ratchet up** — the eight `--at-least` values in the `lint:types:*` scripts sit exactly on the measured figure, with no slack: `any` is banned outright by lint, so this gate exists for the *implicit* kind the lint cannot see — values whose type came from an untyped boundary. Raise a threshold when a change improves the figure; never lower one to make the run pass without explicit approval. The fix is almost always an annotation at the boundary (`const error: unknown = err.error`), which restores narrowing rather than suppressing the metric. `apps/client` is deliberately absent: type-coverage counts every value typed through a `@drevo-web/*` alias as `any` there, so its figure measures the tool rather than the code — issue #254 carries the reproduction
