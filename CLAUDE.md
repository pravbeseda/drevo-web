...
11. **Every file carries its own floor** — `yarn lint:coverage` (`scripts/check-file-coverage.js`) reads the same coverage run as the aggregate and checks each file on its own, so a new component or service with no spec fails by name instead of being absorbed by an aggregate over hundreds of files. Two things to know before touching it:
